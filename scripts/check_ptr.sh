...
#  Description: Checks if the PTR record of the public IP points to MAILCOW_HOSTNAME
...
# --- PTR Lookup ---
# Reverse IP for dig -x
PTR_RESULT=$(dig +short -x "$MAIL_IP" @8.8.8.8 2>/dev/null | sed 's/\.$//' | head -1)
...
# --- Compare: PTR must point to MAILCOW_HOSTNAME ---
