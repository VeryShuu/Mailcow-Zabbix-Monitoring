...
#  Description: Checks TLS certificates on HTTPS/IMAPS/Submission ports
...
case "$CHECK_TYPE" in
    ...
    cert_raw)
        ...
        ;;
    smtp_banner)
        BANNER=$(timeout 5 bash -c "exec 3<>/dev/tcp/$DOMAIN/$PORT; read -t 3 line <&3; echo \$line; exec 3>&-" 2>/dev/null)
        echo "$BANNER" | grep -q "^220" && echo 1 || echo 0
        ;;
    ui_check)
        ...
        ;;
    *)
        echo "Usage: $0 [port_check|cert_days|cert_raw|smtp_banner|ui_check] [port]"
        exit 1
        ;;
esac
