#!/bin/bash
# ====================================================================
#  Mailcow Zabbix Monitoring - Complete Test
#  Version:    1.0
#  Vendor:     Alexander Fox | PlaNet Fox
#  Project:    https://github.com/linuser/Mailcow-Zabbix-Monitoring
#  Description: Tests all 246 UserParameters via zabbix_get
...
echo "============================================="
echo " Mailcow Monitoring v1.0 - Complete Test"
echo " 307 UserParameters / 303 Template Items"
echo "============================================="
echo ""
...
test_key() {
    ...
    if echo "$RESULT" | grep -q "Connection reset by peer"; then
        printf "  ${RED}✗${NC} %-45s ${RED}Connection reset - add 127.0.0.1 to Server= in agent config${NC}\n" "$label"
    ...
    case "$expect_type" in
        number) echo "$RESULT" | grep -qE '^-?[0-9]+\.?[0-9]*$' || { printf "  ${YELLOW}?${NC} %-45s %s\n" "$label" "${YELLOW}$RESULT (expected: number)${NC}"; WARN=$((WARN+1)); return 0; } ;;
        bool)   echo "$RESULT" | grep -qE '^[01]$' || { printf "  ${YELLOW}?${NC} %-45s %s\n" "$label" "${YELLOW}$RESULT (expected: 0/1)${NC}"; WARN=$((WARN+1)); return 0; } ;;
        json)   echo "$RESULT" | grep -qE '^\[|^\{' || { printf "  ${YELLOW}?${NC} %-45s %s\n" "$label" "${YELLOW}(not JSON)${NC}"; WARN=$((WARN+1)); return 0; } ;;
    esac
...
systemctl is-active --quiet zabbix-agent2 || { echo -e "${RED}✗ Agent is NOT running!${NC}"; exit 1; }
echo -e "  ${GREEN}✓${NC} Zabbix Agent 2 running"
PING_RESULT=$(zabbix_get -s 127.0.0.1 -k agent.ping 2>&1)
if echo "$PING_RESULT" | grep -q "Connection reset by peer"; then
...
# [23] Redis (10)
echo -e "${BLUE}=== [23] Redis (10 Keys) ===${NC}\n"
test_key "Running" "mailcow.redis.running" bool
for k in used_memory used_memory_peak keyspace_hits keyspace_misses connected_clients evicted_keys rdb_last_save_time uptime_in_seconds; do
    test_key "Redis $k" "mailcow.redis.$k" number
done
test_key "Hit ratio %" "mailcow.redis.hit_ratio" number
echo ""
...
if [ $FAIL -eq 0 ]; then
    ...
    echo -e "${RED}============================================="
    echo -e " ⚠  $FAIL ERRORS FOUND!"
    echo -e "=============================================${NC}"
    echo "Tips: sudo python3 /usr/local/bin/mailcow-collector.py"
